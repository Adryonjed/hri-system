...
def anci():
    ...
    tit = customtkinter.CTkLabel(f2title, text="ANCILLARY Records", font=("Arial", 30, 'bold'))
    tit.place(x=20,y=20)
    subtit = customtkinter.CTkLabel(f2title, text="manage ancillary records", font=("Arial", 20))
    ...
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM personal WHERE department = 'ANCILLARY' ")
    ...
    def modify_e():
        ...
        sta = customtkinter.CTkOptionMenu(sf3,height= 35, width = 350,fg_color='#a2a3a2',font=('Arial', 22),dropdown_font = ('Courier', 16),dropdown_fg_color ='white',dropdown_text_color = 'black',dropdown_hover_color = 'green', button_color = '#a2a3a2',button_hover_color = 'gray',text_color = "black", variable = staEntry, values=["Cook", "Housekeeper", "Driver", "Porter", "Maintenance Worker"])
    ...
    def read2():
      conn = connection()
      cursor = conn.cursor()
      cursor.execute("SELECT id, firstname, surname, staff, department,status FROM personal WHERE position = %s AND status = %s AND department = 'ANCILLARY' ",(findEntry.get(), findEntry3.get()))
