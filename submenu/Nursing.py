...
def nurse():
    ...
    tit = customtkinter.CTkLabel(f2title, text="NURSING Records", font=("Arial", 30, 'bold'))
    tit.place(x=20,y=20)
    subtit = customtkinter.CTkLabel(f2title, text="manage nursing records", font=("Arial", 20))
    ...
    for g in result:
        ...
        delp.grid(row= i, column = 5,pady=5,padx = 3)
        ...
        showb2.grid(row= i, column = 6,pady=5,padx = 3)
    ...
    def read2():
      conn = connection()
      cursor = conn.cursor()
      cursor.execute("SELECT id, firstname, surname, staff, department,status FROM personal WHERE position = %s AND status = %s AND department = 'NURSING' ",(findEntry.get(), findEntry3.get()))
