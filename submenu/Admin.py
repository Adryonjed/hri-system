...
def adm():
    ...
    tit = customtkinter.CTkLabel(f2title, text="ADMIN Records", font=("Arial", 30, 'bold'))
    tit.place(x=20,y=20)
    subtit = customtkinter.CTkLabel(f2title, text="manage ADMIN records", font=("Arial", 20))
    ...
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM personal WHERE department = 'ADMIN' ")
    ...
    def modify_e():
        ...
        sf3 = customtkinter.CTkFrame(cv3, width=1500, height=2100 ,fg_color ="#d4d4d4")
    ...
    def read2():
      conn = connection()
      cursor = conn.cursor()
      cursor.execute("SELECT id, firstname, surname, staff, department,status FROM personal WHERE position = %s AND status = %s AND department = 'ADMIN' ",(findEntry.get(), findEntry3.get()))
